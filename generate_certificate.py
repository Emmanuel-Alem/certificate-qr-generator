...
def create_certificate(template_path, output_path, name, qr_url):
    ...
    # Center the text
    name_x = (img.width - text_width) // 2 - 180  # Adjusted for left side
    name_y = 365
    
    draw.text((name_x, name_y), name, fill=(76, 255, 162), font=name_font)  # Green color
    
    # Generate and add QR code
    qr_img = generate_qr_code(qr_url, size=180)
    
    # Position QR code (adjust based on your template)
    qr_position = (570, 395)  # Right side of certificate
